...
// Byggd på grafRam (ram, typsnitt, axlar, tooltip, horisontell y-titel).
...
export function linjediagramFacet(data, {
  ...
  // ── Layout ──
  const harLegend = !!(color && showLegend);
  const topRowH = (harLegend || yLabel) ? 30 : 0;
  const marginTop = 10;
  const marginBottom = xLabel ? 20 : 4;
  const outerLeft = 8;
  const outerRight = 8;
  const gapX = 28;
  const gapY = 8;
  ...
  const gridTop = marginTop + topRowH;
  const rowCellYs = [];
  let curY = gridTop;
  ...
  const colorizeSubtitle = (text) => {
    ...
    for (const cv of [...colorValues].sort((a, b) => String(b).length - String(a).length)) {
      ...
      html = html.replace(new RegExp(esc, 'gi'), (m) => { any = true;
        return `<span style="background:${c};color:#fff;padding:1px 7px;border-radius:999px;font-weight:600;font-size:0.92em;white-space:nowrap">${m}</span>`; });
    }
  ...
  if (subtitle && !showLegend) {
    const colored = colorizeSubtitle(subtitle);
    if (colored) ram.subtitleEl.html(colored);
  }
  const svg = ram.svg(autoWidth, totalHeight);
  const tooltip = skapaTooltip(ram.body);

  // ── Y-titel (horisontell, uppe till vänster) + legend ──
  let legendStart = gridLeft;
  if (yLabel) {
    yTitel(svg, { x: outerLeft, y: marginTop + 12, text: yLabel });
    legendStart = Math.max(gridLeft, outerLeft + matText(yLabel, { size: STORLEK.yTitel, weight: 500 }) + 28);
  }
  if (harLegend) {
    const legendG = svg.append("g").attr("transform", `translate(${legendStart}, ${marginTop + 6})`);
    const lineWidth = 20, labelGap = 6, itemGap = 20;
    let lx = 0;
    for (const cv of colorValues) {
      const item = legendG.append("g").attr("transform", `translate(${lx}, 0)`);
      item.append("line")
        .attr("x1", 0).attr("x2", lineWidth).attr("y1", 6).attr("y2", 6)
        .attr("stroke", colorScale(cv)).attr("stroke-width", 2.6).attr("stroke-linecap", "round");
      item.append("text")
        .attr("x", lineWidth + labelGap).attr("y", 6).attr("dy", "0.35em")
        .attr("font-family", TYP.ui).attr("font-size", 12).attr("fill", FARG.ink)
        .text(cv);
      lx += lineWidth + labelGap + matText(cv, { size: 12 }) + itemGap;
    }
  }
  ...
  for (let idx = 0; idx < nFacets; idx++) {
    ...
    bgGroup.append("rect")
      ...
      .attr("fill", "#fbfcfb").attr("stroke", "#e4e6e5").attr("stroke-width", 1)
    ...
    if (showGrid) {
      ys.ticks(4).forEach(tv => {
        ...
        bgGroup.append("line")
          .attr("x1", plotX0 + 1).attr("x2", plotX1 - 1).attr("y1", yPx).attr("y2", yPx)
    ...
    if (isLastInColumn) {
      ...
      xAxisG.call(d3.axisBottom(xs).tickValues(xTickVals).tickFormat(formatX).tickSize(4));
      stilXAxel(xAxisG);
      xAxisG.selectAll(".tick text").attr("font-size", 11);
    } else {
      bgGroup.append("line")
        .attr("x1", plotX0).attr("x2", plotX1).attr("y1", plotY1).attr("y2", plotY1)
        .attr("stroke", FARG.axel).attr("stroke-width", 0.6);
    }

    // Linjer
    const rowData = facetGroups.get(name);
    const byColor = color ? d3.group(rowData, d => d[color]) : new Map([["_all", rowData]]);
    ...
    for (const [cv, rows] of byColor) {
      ...
      const localLine = d3.line()
        ...
        .x(d => xs(d[x])).y(d => ys(d[y])).curve(curve);
    ...
    if (endLabels) {
      ...
      for (let i = 1; i < endItems.length; i++) {
        if (endItems[i].yPos - endItems[i - 1].yPos < minGap) endItems[i].yPos = endItems[i - 1].yPos + minGap;
      }
      for (const it of endItems) {
        const cx = xs(lastX);
        endLabelsGroup.append("circle")
          .attr("cx", cx).attr("cy", ys(it.value)).attr("r", 3)
          .attr("fill", colorScale(it.cv)).attr("stroke", "#fff").attr("stroke-width", 1);
        endLabelsGroup.append("text")
          .attr("x", cx + 6).attr("y", it.yPos).attr("dy", "0.32em")
          .attr("font-family", TYP.ui).attr("font-size", 11).attr("font-weight", 600)
          .attr("fill", colorScale(it.cv)).style("font-variant-numeric", "tabular-nums")
          .text(endLabelText ? endLabelText(it.cv, it.value) : formatY(it.value));
      }
    }
    ...
    facetCells.push({ name, xs, ys, plotX0, plotX1, plotY0, plotY1, sortedByColor });
  }

  // ── Synkad crosshair + tooltip ──
  const crosshairsG = svg.append("g").attr("class", "facet-crosshairs").style("pointer-events", "none");
  ...
  function snapToX(rawX) {
    ...
    const a = allXValues[i - 1], b = allXValues[i];
    return (rawX - a) < (b - rawX) ? a : b;
  }
  function clearOverlay() { crosshairsG.selectAll("*").remove(); tooltip.dolj(); }
  ...
  function drawCrosshair(xVal, hoverCv) {
    ...
    for (const f of facetCells) {
      ...
      crosshairsG.append("line")
        .attr("x1", cx).attr("x2", cx).attr("y1", f.plotY0).attr("y2", f.plotY1)
      ...
      for (const [cv, rows] of f.sortedByColor) {
        ...
        crosshairsG.append("circle")
          ...
          .attr("r", isHover ? 5 : 3).attr("fill", colorScale(cv))
          .attr("stroke", "#fff").attr("stroke-width", isHover ? 2 : 1.2)
  ...
  function showCategoryTooltip(hoverFacet, xVal, hoverCv, event) {
    ...
    for (const f of facetCells) {
      ...
      if (!rows) continue;
      const row = rows.find(r => r[x] === xVal);
      if (row) items.push({ region: f.name, value: row[y], isHovered: f.name === hoverFacet.name });
    }
    items.sort((a, b) => b.value - a.value);
    const headerColor = colorScale(hoverCv);
    const rubrik = color
      ? `<span style="color:${headerColor}">${hoverCv}</span><span style="color:${FARG.mjuk};font-weight:400;margin-left:8px">${formatX(xVal)}</span>`
      : formatX(xVal);
    const rader = items.map(it => ({ namn: it.region, varde: formatY(it.value), farg: headerColor, fokus: it.isHovered }));
    const b = ram.body.node().getBoundingClientRect();
    tooltip.visa(tooltipHtml(rubrik, rader), { x: event.clientX - b.left, y: event.clientY - b.top });
  }
    ...
    for (const f of facetCells) {
      overlayG.append("rect")
        ...
        .on("mousemove", function (event) {
          ...
          const xVal = snapToX(f.xs.invert(mx));
          let nearestCv = null, nearestDist = Infinity;
          for (const [cv, rows] of f.sortedByColor) {
            const row = rows.find(r => r[x] === xVal);
            if (!row) continue;
            const dist = Math.abs(f.ys(row[y]) - my);
            if (dist < nearestDist) { nearestDist = dist; nearestCv = cv; }
          }
  ...
  if (xLabel) xTitel(svg, { x: gridLeft + gridWidth / 2, y: totalHeight - 4, text: xLabel });
